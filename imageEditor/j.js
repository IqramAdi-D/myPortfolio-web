...
filterOptions.forEach(option => {
  option.addEventListener("click", () => {
    ...
    // untuuk menseting nilai dari masing" filter pada saat diganti filter settingannya tidak berubah
...
rotateOptions.forEach(option => {
  option.addEventListener("click", () => { //adding click event listener to all rotate/flip buttons
    if(option.id === "left"){
      rotate -= 90; //if clicked btn is left rotate, decrement rotate value by -90deg
    }else if(option.id === "right"){
      rotate += 90; // this is will be rotate to right, increment rotate value by +90deg.
...
const resetFilter = () => {
  ...
  filterOptions[0].click(); // clicking brightness btn, so the brighness selected by default
  applyFilters();
}
...
const saveImage = () => {
  ...
  // applying user selected filters t canvas filter
  ...
  link.href = canvas.toDataURL(); //passing <a> tag href vlue to canvas data url
